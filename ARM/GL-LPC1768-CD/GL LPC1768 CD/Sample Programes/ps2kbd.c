...
#define kbDAT (LPC_GPIO1->FIOPIN & (KB_DAT))
...
uint8_t init_keyboard(void)
{
	...
	LPC_PINCON->PINSEL3 &= ~(0x0000000C);  // KB_DAT p1.17
	LPC_GPIO2->FIOCLR = KB_CLK;             //hold keyboard clock line untill ready
	LPC_GPIO1->FIODIR &= ~(KB_DAT);

	LPC_PINCON->PINSEL4 &= ~(0x0C000000);
	LPC_PINCON->PINSEL4 |= (0x04000000);	/* set P2.13 as EINT3 and  */  
	//LPC_GPIOINT->IO2IntEnF |= 0x00002000;	/* Port2.13 is falling edge. */
	LPC_SC->EXTMODE |= EINT3_EDGE;		/* INT2 edge trigger */
